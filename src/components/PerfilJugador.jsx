  ...
  return (
    <div className="perfiljugador-wrapper">
      ...
      <div className="cabecera-imagen">
        <div className="perfiljugador-card">
          ...
          <div className="perfiljugador-info">
            <h1 className={`nombre-jugador ${usuario.rango_usuario ? `rango-${usuario.rango_usuario.toLowerCase()}` : ""}`}>
              ...
              {usuario.es_premium && <img src="/assets/premium.png" alt="Premium" className="icono-premium" />}
            </h1>
        ...
        <div className="borde-inferior-imagen" />
      </div>
      ...
      <div className="estadisticas-wrapper-decorada">
        ...
        {statsActuales ? (
          ...
          <p className="mensaje-vacio">Sin estadísticas en este servidor.</p>
        )}
      </div>
      ...
      <div className="sanciones-wrapper">
        <h3>Sanciones</h3>
        {sanciones.length === 0 ? (
          <p className="sin-sanciones">Este jugador es un ejemplo a seguir. No tiene ninguna sanción.</p>
        ) : (
          <ul className="sanciones-lista">
            {sanciones.map((s, i) => (
              <li key={i} className="sancion-item">
                <p><strong>Razón:</strong> {s.razon || "No especificada"}</p>
                <p><strong>Fecha:</strong> {new Date(s.fecha).toLocaleDateString()}</p>
                <p><strong>Staff:</strong> {s.staff || "Desconocido"}</p>
              </li>
            ))}
          </ul>
        )}
